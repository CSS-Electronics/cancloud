/*
 * Javascript Library for Amazon S3 Compatible Cloud Storage, (C) 2015 Storage, Inc.
 *
...
export class Client {
  constructor(params) {
    ...
    //
    //       Storage (OS; ARCH) LIB/VER APP/VER
    //
    var libraryComments = `(${process.platform}; ${process.arch})`
    var libraryAgent = `Storage ${libraryComments} Storage-js/${Package.version}`
  ...
  makeRequest(options, payload, statusCode, region, returnResponse, cb) {
    ...
    if (options.method === 'OPTIONS' || options.method === 'POST' || options.method === 'PUT' || options.method === 'DELETE') {
  ...
  fGetObject(bucketName, objectName, filePath, cb) {
    ...
    async.waterfall([
      ...
      (ignore, cb) => {
        partFile = `${filePath}.${objStat.etag}.part.Storage`
  ...
  // Head Objects
  headObject(bucketName, objectName, cb) {
    if (!isValidBucketName(bucketName)) {
      throw new errors.InvalidBucketNameError('Invalid bucket name: ' + bucketName)
    }
    if (!isValidObjectName(objectName)) {
...
Client.prototype.headObject = promisify(Client.prototype.headObject)
