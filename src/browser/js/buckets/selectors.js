...
const bucketsSelectorMeta = state => state.buckets.bucketsMeta;

export const getVisibleBuckets = createSelector(
  bucketsSelector,
  bucketsFilterSelector,
  (buckets, filter) =>
    buckets.filter(bucket => bucket.indexOf(filter.toUpperCase()) > -1)
);

export const getVisibleBucketsMeta = createSelector(
  bucketsSelectorMeta,
  bucketsFilterSelector,
  (bucketsMeta, filter) =>
    bucketsMeta.filter(
      bucket => bucket.toUpperCase().indexOf(filter.toUpperCase()) > -1
    )
);
